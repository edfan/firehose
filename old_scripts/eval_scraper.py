...
def scrape_class_info(session, class_element, class_dict, term):
    ...
    try:
        for x in range(3, 100):
            ...
            prof['role'] = session.find_element_by_xpath("/html/body/div[@id='contentsframe']/table[@class='grid']/tbody/tr[{}]/td[1]".format(x)).text.replace(prof['name'], '').rstrip('(RLECABDGMO1234567890)').strip(' ,')
